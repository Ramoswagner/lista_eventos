...
function getConfig_() { return CONFIG; }
function getSpreadsheetId_() { return CONFIG.SPREADSHEET_ID; }
...
// ------------------------------------------------------------
function _configObter_(chave, padrao) {
  const linha = dbListar_(DB.CONFIG, c => c.Chave === chave)[0];
  return (linha && linha.Valor) || padrao;
}
function _configSalvar_(chave, valor) {
  const existente = dbListar_(DB.CONFIG, c => c.Chave === chave)[0];
  if (existente) dbAtualizar_(DB.CONFIG, chave, { Valor: valor, Atualizado_Em: new Date() });
  else dbInserir_(DB.CONFIG, { Chave: chave, Valor: valor, Atualizado_Em: new Date() });
}

function apiLogoPublico() {
  const url = _configObter_('LOGO_URL', '');
  return { ok: true, dados: {
    url:           url || CONFIG.LOGO_PADRAO,
    escala:        Number(_configObter_('LOGO_ESCALA', 1)) || 1,
    personalizada: !!url
  } };
}
...
function apiRemoverLogo(token) {
  try {
    ...
    const urlAtual = _configObter_('LOGO_URL', '');
